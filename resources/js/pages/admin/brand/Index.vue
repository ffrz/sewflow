...
const deleteItem = (row) =>
  handleDelete({
    message: `Hapus brand ${row.name}?`,
    url: route("admin.brand.delete", row.id),
...
const onRowClicked = (row) => router.get(route('admin.brand.detail', { id: row.id }));
...
<template>
  ...
  <authenticated-layout>
    ...
    <template #right-button>
      <q-btn icon="add" dense color="primary" @click="router.get(route('admin.brand.add'))" />
                  ...
                  <q-menu anchor="bottom right" self="top right" transition-show="scale" transition-hide="scale">
                    <q-list style="width: 200px">
                      <q-item clickable v-ripple v-close-popup
                        @click.stop="router.get(route('admin.brand.duplicate', props.row.id))">
                      ...
                      <q-item clickable v-ripple v-close-popup
                        @click.stop="router.get(route('admin.brand.edit', props.row.id))">
                        <q-item-section avatar>
                          <q-icon name="edit" />
                        </q-item-section>
